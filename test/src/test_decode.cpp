...
SCENARIO("Arduino Cloud Properties are decoded", "[ArduinoCloudThing::decode]")
{
  ...
  WHEN("Multiple properties is changed via CBOR message")
  {
    GIVEN("CloudProtocol::V1")
    {
      ...
      WHEN("Multiple properties of different type are synchronized via CBOR message. FORCE_DEVICE_SYNC is passed as synchronization function and as a consequence local values of the properties are maintained")
      {
        ArduinoCloudThing thing;
        thing.begin();
        ...
        REQUIRE(bool_test  == false);
        REQUIRE(int_test   == 1);
        REQUIRE(float_test == 2.0f);
        REQUIRE(str_test   == "str_test");
      }
  ...
  WHEN("A payload containing a CBOR BaseVersion is parsed")
  {
    GIVEN("CloudProtocol::V1")
    {
      ...
      /* [{"bver": 1, "n": "test", "v": 1}] = 81 A3 64 62 76 65 72 01 61 6E 64 74 65 73 74 61 76 01 */
      uint8_t const payload[] = {0x81, 0xA3, 0x64, 0x62, 0x76, 0x65, 0x72, 0x01, 0x61, 0x6E, 0x64, 0x74, 0x65, 0x73, 0x74, 0x61, 0x76, 0x01};
      thing.decode(payload, sizeof(payload)/sizeof(uint8_t));
